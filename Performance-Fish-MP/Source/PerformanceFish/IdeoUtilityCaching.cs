...
public class IdeoUtilityCaching : ClassWithFishPatches
{
	...
	public struct StyleDominanceCacheValue : IIsRefreshable<StyleDominanceCache, StyleDominanceCacheValue>
	{
		...
		{
			Value = value;
			_nextRefreshTick = Current.Game.tickManager.TicksGame + 512 + Math.Abs(FishSettings.nextRandInt % 256); // Pawn_StyleObserverTracker.StyleObservableTick runs every 900 ticks
		}
		...
		public bool ShouldRefreshNow
		{
			...
			set => _nextRefreshTick = value ? 0 : Current.Game.tickManager.TicksGame + 512 + Math.Abs(FishSettings.nextRandInt % 256);
		}
