...
public class ThoughtWorker_Precept_AltarSharingOptimization : ClassWithFishPatches
{
	...
	public struct SharedAltarCacheValue : Cache.IIsRefreshable<Ideo, SharedAltarCacheValue>
	{
		...
		public SharedAltarCacheValue(Pawn pawn, Thing? result)
		{
			...
			_nextLateRefreshTick = ticks + 3072 + Math.Abs(FishSettings.nextRandInt % 2048);
			_nextEarlyRefreshTick = ticks + 128 + Math.Abs(FishSettings.nextRandInt % 128);
		...
		public bool ShouldRefreshNow
		{
			...
			{
				var ticks = Current.Game.tickManager.TicksGame;
				_nextLateRefreshTick = value ? 0 : ticks + 3072 + Math.Abs(FishSettings.nextRandInt % 2048);
				_nextEarlyRefreshTick = value ? 0 : ticks + 128 + Math.Abs(FishSettings.nextRandInt % 128);
			}
		}
