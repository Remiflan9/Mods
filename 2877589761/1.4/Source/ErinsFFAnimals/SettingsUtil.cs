...
namespace FFAnimals
{
	public static class SettingsUtil
	{
		...
		public static void SettingsDropdown(this Listing_Standard listing, string name, string explanation, ref TrainabilityEnum value, float width)
		{
			float curHeight = listing.CurHeight;
			Rect rect = listing.GetRect(Text.LineHeight + listing.verticalSpacing);
			Text.Font = GameFont.Small;
			...
			if (Widgets.ButtonText(rect2, value.ToString().Replace("_", " "), true, true, true))
			{
				...
				List<TrainabilityEnum> enumValues = Enum.GetValues(typeof(TrainabilityEnum)).Cast<TrainabilityEnum>().ToList();
				foreach (TrainabilityEnum enumValue in enumValues)
				{
					TrainabilityEnum outValue = value;
