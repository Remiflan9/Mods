        ...
        public static void RecalculateAllGlowPostfix(GlowGrid __instance, Map ___map)
        {
            ...
            for (int i = 0; i < numGridCells; i++)
            {
                ...
                if (position.Roofed(___map))
                {
                    ...
                    if (glow <= 0.6f)
                    {
                        __instance.glowGrid[i] = new ColorInt(Color.Lerp(__instance.glowGrid[i].ToColor, Color.black, 1f - glow));
                        __instance.glowGridNoCavePlants[i] = new ColorInt(Color.Lerp(__instance.glowGridNoCavePlants[i].ToColor, Color.black, 1f - glow));
                    }
                }
            }
